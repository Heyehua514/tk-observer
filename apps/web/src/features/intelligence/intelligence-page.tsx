...
} from './hooks/use-intelligence-items'
import {
...
export function IntelligencePage() {
  ...
  const [status, setStatus] = useState<IntelligenceItem['status'] | 'all'>(
    'all'
  )
  ...
  const confirmTaskIntent = async () => {
    if (!taskItem) return
    try {
      await updateItem.mutateAsync({ id: taskItem.id, status: 'tasked' })
      setTaskItem(null)
      toast.success('已记录转任务意图')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : '转任务状态更新失败')
    }
  }
  ...
  const importCsv = async (file: File) => {
    ...
    if (result.errors.length)
      return toast.error(result.errors.slice(0, 2).join('；'))
    try {
    ...
    <div className='space-y-6'>
      ...
      <div className='grid gap-3 md:grid-cols-[1fr_auto_auto]'>
        ...
        <select
          ...
          value={status}
          onChange={(event) =>
            setStatus(event.target.value as IntelligenceItem['status'] | 'all')
          }
        >
          <option value='all'>全部状态</option>
          {intelligenceStatuses.map((value) => (
            <option key={value} value={value}>
              {statusLabels[value]}
            </option>
          ))}
      ...
      {items.isLoading ? (
        <div className='min-h-40 animate-pulse rounded-xl border bg-muted/30' />
      ) : items.isError ? (
        <LoadStateError
          title='情报加载失败'
        ...
        <EmptyState
          title='还没有可用情报'
          description='先录入一条公开或授权来源的信息，团队就能按工作台筛选使用。'
          action={
            <Button onClick={() => setOpen(true)}>
              <Plus className='size-4' />
              新增第一条
            ...
            <Card key={item.id} className='overflow-hidden'>
              <CardContent className='space-y-4 p-5'>
                ...
                <div className='flex flex-wrap gap-2 text-xs text-muted-foreground'>
                  ...
                  <span>
                    采集于 {new Date(item.capturedAt).toLocaleString('zh-CN')}
                  </span>
                  {item.topic && <span>主题：{item.topic}</span>}
                </div>
                <div className='flex flex-wrap gap-2'>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void setItemStatus(
                        item,
                        item.status === 'saved' ? 'read' : 'saved'
                      )
                    }
                  >
                    <Star className='size-4' />
                    {item.status === 'saved' ? '取消收藏' : '收藏'}
                  </Button>
                  <Button
                    size='sm'
                    variant='outline'
                    disabled={item.status === 'tasked'}
                    onClick={() => setTaskItem(item)}
                  >
                    <Save className='size-4' />
                    {item.status === 'tasked' ? '已转任务' : '转为任务'}
                  </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-2xl'>
          <DialogHeader>
            <DialogTitle>新增情报</DialogTitle>
            <DialogDescription>
              只录入公开、授权或人工确认的来源；链接不会被系统自动抓取。
            </DialogDescription>
          </DialogHeader>
          <div className='grid gap-4 sm:grid-cols-2'>
                ...
                onChange={(event) =>
                  setDraft({
                    ...
                    sourceType: event.target
                      .value as IntelligenceDraft['sourceType'],
                  })
                }
              >
                {intelligenceSourceTypes.map((value) => (
                  <option key={value} value={value}>
                    {sourceLabels[value]}
                  </option>
                ))}
            ...
            <label className='space-y-1 text-sm'>
              ...
              <Input
                ...
                onChange={(event) =>
                  setDraft({ ...draft, capturedAt: event.target.value })
                }
              />
            </label>
            <label className='space-y-1 text-sm'>
              ...
              <Input
                ...
                onChange={(event) =>
                  setDraft({ ...draft, dedupeKey: event.target.value })
                }
              />
            </label>
            <label className='space-y-1 text-sm'>
      ...
      <Dialog
        open={Boolean(taskItem)}
        onOpenChange={(next) => !next && setTaskItem(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>转为任务</DialogTitle>
            <DialogDescription>
              当前版本只记录转任务意图，不会自动创建业务任务。
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant='outline' onClick={() => setTaskItem(null)}>
              取消
            </Button>
            <Button
              onClick={() => void confirmTaskIntent()}
