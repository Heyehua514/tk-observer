...
describe('business dashboard source mappers', () => {
  ...
  it('keeps client, order and social dates stable', () => {
    expect(mapDashboardClient({ id: 'c1', name: '客户', created_at: 'a', updated_at: 'b' })).toEqual({
      id: 'c1',
      name: '客户',
      created: 'a',
      updated: 'b',
    })
    expect(mapDashboardOrder({ id: 'co1', title: '商单', clients: {}, publish_date: null })).toMatchObject({
      clientName: '未知客户',
      publishDate: '',
    })
    expect(mapDashboardSocialPlan({ id: 's1', content: '内容', date: 'd', status: 'planned' })).toEqual({
      id: 's1',
      content: '内容',
      date: 'd',
      status: 'planned',
