...
it('renders blog metrics and article rows with viral status', async () => {
  ...
  await expect.element(screen.getByText('公众号分析已接入对标账号与爆款沉淀，表格统一使用淡斑马纹和紧凑表头。')).toBeInTheDocument()
  await expect.element(screen.getByText('总文章数')).toBeInTheDocument()
  await expect.element(screen.getByText('爆款数')).toBeInTheDocument()
  await expect.element(screen.getByText('TikTok 全域增长拆解')).toBeInTheDocument()
  await expect.element(screen.getByText('爆款', { exact: true })).toBeInTheDocument()
  await expect.element(screen.getByText('标题给出明确增长承诺')).toBeInTheDocument()
})
...
it('shows guided empty state when filters return no rows', async () => {
  ...
  await expect.element(screen.getByText('还没有公众号文章记录')).toBeInTheDocument()
  await expect.element(screen.getByText('先录入一篇文章，系统会自动统计爆款状态、阅读表现和分析笔记。')).toBeInTheDocument()
})
